...
function get_predictions(state, ml, bs, enc, dec)
	...
	for _, c_batch in pairs(state.batches) do
		...
		if #prediction == 0 then
			...
			table.insert(all_alphas, JSON:encode(torch.totable(alphas)))
	...
	return {predictions, alignments, all_alphas}
...
while cnt < max_length do
	...
	for i = 1, #beams do
		...
		if beams[i].x[1] ~= 4 then
			...
			local attention_vector = fnodes[50].data.mapindex[1].input[2]
			attention_vector = attention_vector:view(attention_vector:nElement())
			...
			for j = 1, beam_size do
				...
				local next_alphas = clone(beams[i].alphas)
				...
				if word ~= "UNK" and (id == 4 or ind == 3 or #word < 3 or beams[i].strmap[word] == nil) then
					...
					if ind ~= 4 and ind ~= 3 then
						...
						if next_alphas == nil then
							next_alphas = attention_vector
						else
							next_alphas = torch.cat(next_alphas,attention_vector,2)
						end
						next_map[word] = true
					end
...
return {beams[1].str, beams[1].alignments, beams[1].alphas}
end
...
function main()
    ...
    cmd:option('-dataset', '', 'Dataset name (without .pickle)')
	...
	local encoderFileName = working_dir .. '/' .. opt.encoder
	local decoderFileName = working_dir .. '/' .. opt.decoder

	vocab = torch.load(vocabFileName)
	state_test = torch.load(testFileName)
	encoderCell = torch.load(encoderFileName)
	decoderCell= torch.load(decoderFileName)
	...
	local predictions, alignments, alphas  = unpack(get_predictions(state_test, params.max_length, params.beam_size, encoderCell, decoderCell))
	...
	local tmpFilename = os.tmpname()
	local tf = io.open(tmpFilename, 'w')
	for _, line in ipairs(alphas) do
		tf:write(tostring(line) .. '\n')
	end
	tf:close()
	print('Attentions in  ' .. tmpFilename)
...
if script_path() == "./model/predict.lua" then
